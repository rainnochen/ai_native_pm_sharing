...
这个仓库配套分享《AI 原生组织下的 PM 自进化实践》，目标不是整理一份 AI 工具大全，而是把 PM 可以直接使用的工作资产沉淀下来：上下文模板、Prompt、Skill、Harness、Loop、Hackathon 工具包和 Demo Starter。
...
5. 用 [`loops/pm-loop-canvas.md`](loops/pm-loop-canvas.md) 设计下一轮反馈。
...
```text
Prompt Engineering -> Harness Engineering -> Loop Engineering -> Builder PM
```
