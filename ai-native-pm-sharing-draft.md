...
这意味着 PM 原本就不是文档岗位。PRD、流程图、原型和汇报都只是手段，底层任务始终是：
...
---
# 16. 分享 Slides 初步结构（33 页展示版 / 40 页备稿版）

> 这部分只是从本文抽取的演讲骨架，不是最终逐字稿。当前 HTML 展示版为 **33 页 / 60 分钟**：投屏内容少，重点靠口播强调观点、非共识和个人判断；下面的 40 页结构作为扩展备稿和后续 Slidev 版本参考。

## 时间分配

| 模块 | 页码 | 建议时间 |
|---|---:|---:|
| Opening：为什么要做这次分享 | 1–3 | 5 分钟 |
| 认知底座：AI 原生组织、PM 新定义、大模型基础 | 4–11 | 10 分钟 |
| 大厂案例与组织变化 | 12–16 | 10 分钟 |
| Prompt → Harness → Loop | 17–26 | 20 分钟 |
| Builder / Hackathon / Toolkit | 27–31 | 10 分钟 |
| 行动清单与 Closing | 32–33 | 5 分钟 |
...
| 4 | 什么是 AI 原生组织 | 不是用了 AI 工具，而是把 AI 作为默认生产力、协作对象、执行系统 |
| 5 | PM 的定义如何变化 | 从理解用户与设计交易，升级为设计人机工具数据反馈协作系统 |
| 6 | 传统软件开发 vs AI 原生软件开发 | 从需求-研发-测试，变成意图-上下文-工具-评估-Human Gate-Loop |
| 7 | 大模型为什么会“猜” | 模型不是数据库；它会基于上下文预测和补全 |
| 8 | 互动 Demo：先问我，再回答 | 让模型先问问题、暴露假设、列缺口 |
| 9 | PM 工作台：从 Prompt 到 Harness | Prompt 是对话，Harness 是工作台 |
| 10 | AI 对 PM 的影响：不是提效，是重构 | PRD 写得快不是终点，信息、判断、执行、反馈的连接方式变了才是本质 |
| 11 | 2023-2026：从聊天框到工作系统 | 行业焦点从生成能力迁移到 Agent、Skills、Harness、Loop |
| 12 | AI Native 工作系统四层 | PM 最容易停在模型层，价值在 Harness、Loop 和治理层 |
| 13 | 市场信号：从 Copilot 到工作环境 | 公开实践正在从个人助手走向组织工作环境 |
| 14 | 大厂案例总览 | 公开实践共同指向组织级 Context、Skills、连接器和评估 |
| 15 | 坏指标：不能只看 AI 生成量 | 生成占比不等于端到端交付效率 |
| 16 | 美团：复杂度不会自动收敛 | 人类共识要变成机器可执行规则与验证 |
| 17 | 主线：Prompt → Harness → Loop | 一次回答、可靠任务、持续进化 |
| 18 | Prompt：把一次问题问清楚 | 好 Prompt 的本质是好任务定义 |
| 19 | 为什么“会写 Prompt”已经不够 | 没有状态、工具、权限、验收和反馈，Prompt 只能优化一次输出 |
| 20 | Harness：给 AI 一套工作环境 | 不是 Prompt 库，而是上下文、Skills、工具、状态、规则、Eval |
| 21 | Loop：让反馈闭环跑起来 | 受控重复，并在每轮获得新证据 |
| 22 | 什么时候不要做 Loop | Loop 不是免费的，重复、可验证、可复现、有人 review 才值得自动化 |
| 23 | 控制论：目标、反馈、调节 | PM 对决策负责，必须设计反馈和纠偏 |
| 24 | 信号论：信息接收与验证系统 | 壁垒不是拥有更多信息，而是识别、验证和使用信号 |
| 25 | Loop Contract：先写合同，再自动化 | 触发、目标、范围、工具、验证、停止、升级、状态写回 |
| 26 | 一个需求分析 Loop | 信号进入、AI 处理、PM 判断、验证、记录、更新 |
| 27 | 为什么 PM 必须成为 Builder | 当 Demo 成本下降，PM 不能只做需求传递者 |
| 28 | 代码便宜后，取舍变贵 | AI 会放大功能欲望，PM 要守住场景边界和信任债务 |
| 29 | AI-Native PM 五阶段 | 从 AI User 到 Builder & Loop Owner |
| 30 | Hackathon：从表达者到创造者 | 36 小时把 PM 推到对可运行结果负责的位置 |
| 31 | GitHub Toolkit：可复制的 PM Workbench | 每个 PM 都可以复制自己的 Context、Skills、Loops、Builder Starters |
| 32 | 7 天 PM 自进化挑战 | 先跑一条小而真实的闭环 |
| 33 | Closing：PM 对决策负责 | 不要只使用 AI，开始设计共同进化的系统 |
...
4. 全套控制在 30–34 页，目标演讲时间 60 分钟；复杂资料放入 Speaker Notes、GitHub Toolkit 或附录，不挤进投屏页。
...
以底稿第 16 章的 33 页展示版大纲为基础。允许为了节奏合并或拆分 2 页，但不得改变故事线：

Opening → AI 原生组织 → PM 新定义 → 大模型基础与交互 Demo → 大厂实践 → Prompt/Harness/Loop → Builder → Hackathon → GitHub Toolkit → 7 天行动 → Closing。
