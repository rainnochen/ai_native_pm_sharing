...
核心文件：
- ai-native-pm-sharing-final.html：最终投屏 HTML slides
- ai-native-pm-sharing-final-script.md：最终逐字稿
...
1. Inspect
   - 阅读 ai-native-pm-sharing-final-script.md 的相关章节。
   - 阅读 ai-native-pm-sharing-final.html 的对应 slides。
...
4. Implement
   ...
   - 不写入百度内部敏感信息、真实密钥、Cookie、需授权截图或私人信息。

5. Validate
   - 运行 npm run validate。
   - 运行 npm run check:links。
   - 运行 npm run check:html。
   - 检查 ai-native-pm-sharing-final.html 中中文显示、页数和主要内容。
